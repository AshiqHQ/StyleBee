...
<section class="product-single container">
    ...
    <div class="col-lg-5">
        ...
        <span class="current-price">
            @if ($product->sale_price)
                <s>BDT: {{ $product->regular_price }}</s> BDT: {{ $product->sale_price }}
            @else
        ...
        @if(Cart::instance('cart')->content()->where('id', $product->id)->count() > 0)
            <a href="{{ route('cart.index') }}" class="btn btn-warning mb-3">View in Cart</a>
        @else
    ...
    <div class="tab-content">
        ...
        <div class="product-single__reviews-list">
            ...
            <div class="product-single__reviews-item">
                <div class="customer-avatar">
                    <img loading="lazy" src="{{ asset('assets/images/avatar.png') }}" alt="" />
                </div>
                        ...
                        <div class="reviews-group d-flex">
                            @for($i = 1; $i <= 5; $i++)
                                <svg class="review-star" viewBox="0 0 9 9" xmlns="http://www.w3.org/2000/svg">
                                    <!-- If star index is less than or equal to rating, color it gold -->
                                    <use href="#icon_star" style="fill: {{ $i <= $review->rating ? '#FFD700' : '#ccc' }};" />
                                </svg>
                            @endfor
                        </div>
                    </div>
        ...
        <div class="product-single__review-form">
            <form name="customer-review-form" method="POST" action="{{ route('shop.review.store') }}">
                @csrf
                ...
                <div class="select-star-rating">
                    ...
                    <span class="star-rating">
                        @for($i = 1; $i <= 5; $i++)
                            <svg class="star-rating__star-icon clickable-star" data-value="{{ $i }}" width="12" height="12" fill="#ccc" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg" style="cursor:pointer;">
                    ...
                    <!-- This input stores the actual rating value -->
...
<section class="products-carousel container">
    ...
    <div class="swiper-container js-swiper-slider" data-settings='{
        ...
        <div class="swiper-wrapper">
            @foreach ($rproducts as $rproduct) {{-- Use $rproduct here --}}
            <div class="swiper-slide product-card">
                <div class="pc__img-wrapper">
                    <a href="{{ route('shop.product.details', ['product_slug' => $rproduct->slug]) }}">
                        ...
                        @foreach (explode(',', $rproduct->images) as $gimg)
                            <img loading="lazy" src="{{ asset('uploads/products') }}/{{ $gimg }}" width="330" height="400" class="pc__img pc__img-second">
                        @endforeach
                    </a>
                    @if(Cart::instance('cart')->content()->where('id', $rproduct->id)->count() > 0)
                        <a href="{{ route('cart.index') }}" class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium btn-warning mb-3">View in Cart</a>
                    @else
                    <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
                        @csrf
                ...
                <div class="pc__info position-relative">
                    <p class="pc__category">{{ $rproduct->category->name }}</p> {{-- Fixed extra < and variable name --}}
                    ...
                    <div class="product-card__price d-flex">
                        <span class="money price">
                            @if ($rproduct->sale_price)
                                <s>BDT: {{ $rproduct->regular_price }}</s> BDT: {{ $rproduct->sale_price }}
                            @else
                                BDT: {{ $rproduct->regular_price }}
                            @endif
                        </span>
                    </div>
                </div>
...
<script>
    document.querySelectorAll('.clickable-star').forEach(star => {
        star.addEventListener('click', function() {
            ...
            document.querySelectorAll('.clickable-star').forEach(s => {
                if(s.getAttribute('data-value') <= rating) {
                    s.setAttribute('fill', '#FFD700'); // Gold
                } else {
                    s.setAttribute('fill', '#ccc'); // Gray
                }
            });
        });
    });
